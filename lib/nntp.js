...
function readCode(chunk, code) {
    var ret = code,
        more, left = chunk.length - chunk.p;
    if (left >= 3 && code === undefined) {
        ...
        if (code === undefined) {
            ...
            if (ret.length === 3) ret = parseInt(ret, 10);
        }
    }
    return ret;
}
...
NNTP.prototype.connect = function (options) {
    ...
    var debug;
    if (typeof options.debug === 'function') debug = this._debug = options.debug;
    else debug = this._debug = false;
    ...
    if (this.options.secure) socket = tls.connect({
        socket: this._socket,
        servername: this.options.host
    }, onconnect);
    else this._socket.once('connect', onconnect);
    ...
    socket.on('data', function (chunk) {
        ...
        while (r < chlen) {
            if (typeof code !== 'number') {
                ...
                if (code === 211) isML = (self._curReq.cmd !== 'GROUP');
                else isML = (RES_CODE_ML.indexOf(code) > -1);
                ...
                if (sbmh.matches === 1) {
                    if (self._stream) {
                        if (isErr) self._stream.emit('error', makeError(ERRORS[code], code));
                        else self._stream.emit('end');
                        self._stream.emit('close', isErr);
                    } else if (isErr) self._curReq.cb(makeError(ERRORS[code], code));
                    else {
    ...
    function responseHandler(isMatch, chunk, start, end) {
        if (isErr || !chunk) return;
        if (self._stream === undefined) self._buffer += chunk.toString(self._bufferEnc || 'utf8', start, end);
        else self._stream.emit('data', chunk.slice(start, end));
    }
    ...
    this._send('HEAD', what, function (err, code, r, type) {
        ...
        for (var i = 0, h, len = list.length; i < len; ++i) {
            ...
            if (list[i][0] === '\t' || list[i][0] === ' ') {
                ...
                h = m[1].toLowerCase();
                if (m[2]) {
                    if (headers[h] === undefined) headers[h] = [m[2]];
                    else headers[h].push(m[2]);
                } else headers[h] = '';
            }
        }
...
NNTP.prototype.overviewFormat = function (cb) {
    var self = this;
    this._send('LIST', 'OVERVIEW.FMT', function (err, code, r, type) {
        if (err) {
            return cb(err);
        }

        var lines = self._buffer.split(RE_CRLF);
        lines.shift();
        var format = [];
        lines.forEach(function (line) {
            if (line.substr(-5, 5).toLowerCase() === ':full') {
                format[line.slice(0, -5).toLowerCase().replace(/(-.)/g, function (x) {
                    return x[1].toUpperCase()
                })] = true;
            } else {
                format[line.slice(0, -1).toLowerCase().replace(/(-.)/g, function (x) {
                    return x[1].toUpperCase()
                })] = false;
            }
        });
...
NNTP.prototype.xover = function (firstArticle, lastArticle, format, cb) {
    var self = this;
    this._send('XOVER', firstArticle + '-' + lastArticle, function (err, code, r, type) {
        if (err) {
            return cb(err);
        }
        var headers = [];
        ...
        cb(undefined, headers);
    });
};

NNTP.prototype.newNews = function (search, date8, time6, cb) {
    if (typeof search !== 'string') throw new Error('Expected search string');
/*if (typeof date8 === 'function'
    ...
    if (typeof time6 === 'function') {
        ...
        if (util.isDate(date8)) {
            time6 = padLeft('' + date8.getUTCHours(), 2, '0') + padLeft('' + date8.getUTCMinutes(), 2, '0') + padLeft('' + date8.getUTCSeconds(), 2, '0');
            date8 = '' + date8.getUTCFullYear() + padLeft('' + date8.getUTCMonth(), 2, '0') + padLeft('' + date8.getUTCDate(), 2, '0');
        } else time6 = '000000';
    }
    ...
    this._send('NEWNEWS', search + ' ' + date8 + ' ' + time6 + ' GMT', function (err, code, r, type) {
        if (err) return cb(err);
        var list = self._buffer.split(RE_CRLF);
        list.shift(); // remove initial response line
        cb(undefined, list);
...
NNTP.prototype.groupsDesc = function (search, cb) {
    ...
    if (typeof search === 'function') {
        cb = search;
        search = '';
    } else if (Array.isArray(search)) search = search.join(',');
    search = (search ? ' ' + search : '');
...
NNTP.prototype.post = function (msg, cb) {
    ...
    this._send('POST', function reentry(err, code, r, type) {
        if (err || !composing) return cb(err);
        ...
        text += (Buffer.isBuffer(msg.body) ? msg.body.toString('utf8') : msg.body).replace(/\r\n/g, '\n').replace(/\r/g, '\n').replace(/\n/g, '\r\n').replace(/^\.([^.]*?)/gm, '..$1');
...
NNTP.prototype._send = function (cmd, params, cb) {
    if (cmd !== undefined) this._queue.push({
        cmd: cmd,
        params: params,
        cb: cb
    });
    if (!this._curReq && this._queue.length) {
        ...
        if (this._curReq.params !== undefined) {
            this._debug && this._debug('> ' + this._curReq.cmd + ' ' + this._curReq.params);
            this._socket.write(' ');
...
ReadStream.prototype._emitData = function (d) {
    ...
    } else if (this.paused) this._buffer.push(d);
    else if (this._decoder) {
        var string = this._decoder.write(d);
        if (string.length) this.emit('data', string);
    } else this.emit('data', d);
};
...
ReadStream.prototype.resume = function () {
    if (this._buffer && this._buffer.length) while (this._emitData());
    this.paused = false;
    this._sock.resume();
};
